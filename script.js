...
countryCodes.sort((a, b) => {
  const codeA = parseInt(a.dial_code.replace('+', ''), 10);
  const codeB = parseInt(b.dial_code.replace('+', ''), 10);
  return codeA - codeB;
});

const data = JSON.stringify(countryCodes, null, 2);

// Write the JSON string to a text file
fs.writeFile('sortedCountryCodes.txt', data, (err) => {
  if (err) {
